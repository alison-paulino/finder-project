...
routerRecruiter.post('/createRecruiter', fileUploader.single('image'), (req, res, next) => {
	...
	if (!lastName || !city || !password || !phone || !company) {
		res.render('recruiter/cadastroRecruiter', {
			currentUser,
...
routerRecruiter.get('/editRecruiter/:id', (req, res) => {
	...
	Recruiter.findById(id).then((dadosList) => {
		if (!req.session.currentUser) {
			res.redirect('/');
			return;
		}
...
routerRecruiter.post('/editRecruiter/:id', fileUploader.single('image'), (req, res, next) => {
	...
	if (!lastName || !city || !password || !phone || !company | !name) {
		res.render('recruiter/editProfileRecruiter', {
			currentUser,
			errorMessage: 'Todos os campos são mandatorios. Por favor verifique o preenchimento',
		});
		return;
...
routerRecruiter.post('/listJobs/:id', (req, res) => {
	...
	Job.find({ recruiter_id: id })
		...
		.catch((err) => {
			res.render('recruiter/profileRecruiter', {
				currentUser,
				errorMessage: 'Nenhuma vaga a ser exibida, crie novas vagas!',
			});
		});
});
